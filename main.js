...
var require_lodash = __commonJS({
  "node_modules/lodash.isregexp/index.js"(exports2, module2) {
...
// node_modules/postcss-sanitize/index.js
var require_postcss_sanitize = __commonJS({
  "node_modules/postcss-sanitize/index.js"(exports2, module2) {
    "use strict";
    var valueParser = require_lib();
    var isRegExp = require_lodash();
    var passRule = (rule, decl) => {
    ...
    var deleteEmptyRules = (rule) => {
      if (rule.nodes.length === 0) {
        rule.remove();
      }
    };
    var cleanURLRE = /^\s+/;
    var deleteURL = (url, schemes) => {
        ...
        Once(css, { result }) {
          ...
          if (opts.allowedSchemes) {
            css.walkAtRules("import", (rule) => {
              let url = rule.params.replace(startParenthesis, "").replace(endParenthesis, "");
...
// node_modules/@yaireo/relative-time/relative-time.min.js
var require_relative_time_min = __commonJS({
  "node_modules/@yaireo/relative-time/relative-time.min.js"(exports2, module2) {
    !function(e, t) {
      var o = o || {};
      "function" == typeof o && o.amd ? o([], t) : "object" == typeof exports2 && "object" == typeof module2 ? module2.exports = t() : "object" == typeof exports2 ? exports2.RelativeTime = t() : e.RelativeTime = t();
    }(exports2, function() {
      const e = { year: 31536e6, month: 2628e6, day: 864e5, hour: 36e5, minute: 6e4, second: 1e3 }, t = "en", o = { numeric: "auto" };
      function n(e2) {
        e2 = { locale: (e2 = e2 || {}).locale || t, options: { ...o, ...e2.options } }, this.rtf = new Intl.RelativeTimeFormat(e2.locale, e2.options);
      }
      return n.prototype = { from(t2, o2) {
        const n2 = t2 - (o2 || /* @__PURE__ */ new Date());
        for (let t3 in e) if (Math.abs(n2) > e[t3] || "second" == t3) return this.rtf.format(Math.round(n2 / e[t3]), t3);
      } }, n;
    });
  }
});
...
var require_globals = __commonJS({
  "src/globals.js"() {
    ...
    globalThis.sleep = (ms) => new Promise((bingbong) => setTimeout(bingbong, ms));
    globalThis.is_truthy = (value) => !!value;
    globalThis.is_branch = (ref) => ref.type === "branch";
    var debounce_timeout_map = {};
    globalThis.debounce = (fun, time) => {
...
var require_git = __commonJS({
  "src/git.js"(exports2, module2) {
    ...
    var { basename, relative, isAbsolute } = require("path");
    var { existsSync } = require("fs");
    var { realpath } = require("fs").promises;
    var exec = util.promisify(require("child_process").exec);
    module2.exports.get_git = function(EXT_ID2, logger2, { on_repo_external_state_change, on_repo_names_change }) {
      let api = vscode2.extensions.getExtension("vscode.git")?.exports.getAPI(1) || (() => {
        throw "VSCode official Git Extension not found, did you disable it?";
      })();
      let last_git_execution = 0;
      let repo_state_cache = {};
      function start_observing_repo(repo) {
        logger2.info("start observing repo " + repo.rootUri.fsPath);
        return repo.state.onDidChange(() => {
          ...
          if (api.repositories.findIndex((r) => r.rootUri.path === repo.rootUri.path) !== selected_repo_index)
            return;
          logger2.info("repo watcher: external index/head change");
      ...
      function repos_changed() {
        if (api.repositories.length === repos_cache.length)
          return;
        debounce(() => {
          logger2.info("workspace: repo(s) added/removed");
          api.repositories.filter((repo) => !repos_cache.includes(repo)).forEach((repo) => start_observing_repo(repo));
      ...
      return {
        ...
        get_repo_names() {
          return Object.entries(api.repositories.reduce((all, repo) => {
            let base = basename(repo.rootUri.path);
            all[base] ||= [];
            all[base].push(repo);
            return all;
          }, {})).map(([base, repos]) => repos.length === 1 ? base : repos.map((repo) => repo.rootUri.path)).flat();
        },
        get_repo(repo_index) {
          if (repo_index == null)
        ...
        async run(args, repo_index) {
          let cwd = vscode2.workspace.getConfiguration(EXT_ID2).get("folder");
          let cmd = vscode2.workspace.getConfiguration(EXT_ID2).get("git-path") || vscode2.workspace.getConfiguration("git").get("path") || "git";
          if (!cwd) {
        ...
        set_selected_repo_index(index) {
          logger2.info("set selected repo index " + index);
          selected_repo_index = index;
        },
        get_selected_repo_index: () => selected_repo_index,
        async get_repo_index_for_uri(uri) {
          if (!existsSync(uri.fsPath))
            return -1;
          let uri_path = await realpath(uri.fsPath).catch((e) => {
            throw new Error(e);
          });
          return (await Promise.all(api.repositories.map(async (repo, index) => {
            let repo_path = await realpath(repo.rootUri.fsPath).catch((e) => {
              throw new Error(e);
            });
          ...
          }).sort((a, b) => b.repo_path.length - a.repo_path.length).at(0)?.index ?? -1;
        }
      };
    };
  }
...
var require_logger = __commonJS({
  "src/logger.js"(exports2, module2) {
    "use strict";
    var vscode2 = require("vscode");
    module2.exports = (ext_name, ext_id) => {
      let channel = vscode2.window.createOutputChannel(ext_name);
      function log(msg) {
        channel.appendLine(`${(/* @__PURE__ */ new Date()).toISOString()} ${msg}`);
      }
      return {
        error(e) {
          console.error(e);
          ...
          vscode2.window.showErrorMessage(`${ext_name}: ${e}`);
          log(`[ERROR] ${e}`);
        },
        info(msg) {
          log(`[info] ${msg}`);
        ...
        debug(msg) {
          if (!vscode2.workspace.getConfiguration(ext_id).get("verbose-logging"))
            return;
          log(`[debug] ${msg}`);
        }
      };
...
var require_state = __commonJS({
  "src/state.js"(exports2, module2) {
    "use strict";
    module2.exports.get_state = ({ context, git, on_broadcast }) => {
      let state_change_listeners = {};
      function global_state_memento(key) {
        return {
          get: () => context.globalState.get(key),
          set: (v) => context.globalState.update(key, v)
        };
      }
      function repo_state_memento(local_key) {
        function key() {
        ...
        return {
          ...
          set: (v) => context.workspaceState.update(key(), v)
        };
      }
      ...
      let special_states = {
        // "Normal" states instead are just default_memento
        "selected-repo-index": {
      ...
      return {
        state,
        add_state_change_listener(key, listener) {
...
var postcss_sanitize = require_postcss_sanitize();
var RelativeTime = require_relative_time_min();
var relative_time = new RelativeTime();
require_globals();
var { get_git } = require_git();
var create_logger = require_logger();
var { get_state } = require_state();
var EXT_NAME = "GitLG";
var EXT_ID = "git-log--graph";
var START_CMD = EXT_ID + ".start";
var BLAME_CMD = EXT_ID + ".blame-line";
var webview_container = null;
var logger = create_logger(EXT_NAME, EXT_ID);
...
module.exports.activate = intercept_errors(function(context) {
  ...
  async function populate_webview() {
    ...
    view.onDidReceiveMessage(intercept_errors((message) => {
      logger.debug("receive from webview: " + JSON.stringify(message));
      let data = message.data;
      async function h(func) {
        let resp = {
      ...
      switch (message.type) {
        case "request-from-web":
          switch (message.command) {
            case "git":
              return h(() => git.run(data.args).catch((e) => {
                if (data.ignore_errors)
                  return;
                throw e;
              }));
            case "show-error-message":
              return h(() => logger.error(data));
            case "show-information-message":
              return h(() => vscode.window.showInformationMessage(data));
            case "get-config":
              return h(() => vscode.workspace.getConfiguration(EXT_ID));
            case "get-state":
              return h(() => state(data).get());
            case "set-state":
              return h(() => state(data.key).set(data.value, { broadcast: false }));
            case "open-diff":
              return h(() => {
            ...
            case "open-multi-diff":
              return h(() => vscode.commands.executeCommand(
                "vscode.changes",
            ...
            case "clipboard-write-text":
              return h(() => vscode.env.clipboard.writeText(data));
          }
      }
    }));
    ...
    let csp = `default-src 'none'; style-src ${view.cspSource} 'unsafe-inline' ` + (is_production ? "" : dev_server_url) + `; script-src ${view.cspSource} 'unsafe-inline' ` + (is_production ? "" : `${dev_server_url} 'unsafe-eval'`) + `; font-src ${view.cspSource} ` + (is_production ? "" : dev_server_url) + "; connect-src " + (is_production ? "" : "*") + `; img-src ${view.cspSource} ` + (is_production ? "" : dev_server_url) + "; ";
    let base_url = is_production ? view.asWebviewUri(vscode.Uri.joinPath(context.extensionUri, "web-dist")) + "/" : dev_server_url;
    let custom_css = vscode.workspace.getConfiguration(EXT_ID).get("custom-css");
    if (custom_css)
      custom_css = await postcss([postcss_sanitize({})]).process(custom_css, { from: void 0 }).then((c) => c.css).catch(() => 0);
    let loading_prompt = is_production ? "Loading (this should not take long)" : 'Trying to connect to dev server... See CONTRIBUTING.md > "Building" for instructions';
    view.html = `
			<!DOCTYPE html>
			<html lang='en'>
			<head>
  ...
  context.subscriptions.push(vscode.workspace.registerTextDocumentContentProvider(`${EXT_ID}-git-show`, {
    provideTextDocumentContent: intercept_errors(
      (uri) => git.run(`show "${uri.path}"`).catch(() => "")
    )
  }));
  ...
  function show_blame(text_editor) {
    let doc = text_editor.document;
    let uri = doc.uri;
    if (uri.scheme !== "file" || doc.languageId === "log" || doc.languageId === "Log" || uri.path.includes("extension-output") || uri.path.includes(EXT_ID))
      return;
    ...
    line_change_debouncer = setTimeout(intercept_errors(async () => {
      ...
      let time = relative_time.from(new Date(Number(blamed[3]?.slice(12)) * 1e3));
